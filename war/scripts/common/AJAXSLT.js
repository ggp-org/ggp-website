// ============================= =========================
// This file is based on the open source AJAXSLT codebase.
// at http://code.google.com/p/ajaxslt/
// ============================= =========================
...
function AJAXSLT_find_entities_to_replace(input) {
  var expr = "\<\!ENTITY ([^\'\"]*) [\'\"]([^\'\"]*)[\"\']\>";
  
  var q = input.match(expr);
  if (!q) return [];
  var in2 = input.replace(q[0], "");
  var t = AJAXSLT_find_entities_to_replace(in2);
  t.push([q[1],q[2]]);
  return t;
}

function AJAXSLT_replace_entities(xml) {
  var to_replace = AJAXSLT_find_entities_to_replace(xml);
  
  for(var i = 0; i < to_replace.length; i++) {
    xml = xml.replace(new RegExp("&" + to_replace[i][0] + ";", 'g'), to_replace[i][1]);
  }
...
function run_AJAXSLT_Processor(xml_in, xslt_raw) {
  var xml = xmlParse(xml_in.innerHTML);
  var xslt = xmlParse(AJAXSLT_replace_entities(xslt_raw));
  var html = xsltProcess(xml, xslt);

  var xsltResult = document.createElement('xslt_result');
  xsltResult.innerHTML = html;
  return xsltResult;
}